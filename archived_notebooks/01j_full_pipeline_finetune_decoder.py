...
def create_sequences(df: pd.DataFrame):
    ...
    padded_sequences = np.zeros((len(sequences), FIXED_SEQ_LEN, len(input_features)), dtype=np.float32)
...
# ==========================================
# 4. GAN ARCHITECTURE & PHYSICS LOSS (WITH DECODER FINETUNING)
# ==========================================
def build_generator() -> Model:
    """Generator relies on dropout for stochasticity, taking only cond as input."""
    cond_input = layers.Input(shape=(COND_DIM,))
    x = layers.Dense(64, activation='relu')(cond_input)
    x = layers.Dropout(0.2)(x)
    x = layers.Dense(64, activation='relu')(x)
    x = layers.Dropout(0.2)(x)
    latent = layers.Dense(LATENT_DIM)(x)
    return Model(cond_input, latent, name="Generator")
...
def build_discriminator() -> Model:
    ...
    return Model([latent_input, cond_input], score, name="Discriminator")

def gradient_penalty(discriminator, real_latent, fake_latent, cond):
    alpha = tf.random.uniform([real_latent.shape[0], 1], 0., 1.)
    interpolated = alpha * real_latent + (1 - alpha) * fake_latent
...
def train_gan_with_finetuning(X_real, y_cond, decoder):
    # Normalize conditions
    mean, std = np.mean(y_cond), np.std(y_cond)
    y_cond_norm = (y_cond - mean) / std
    
    dataset = tf.data.Dataset.from_tensor_slices((X_real, y_cond_norm)).shuffle(10000).batch(BATCH_SIZE).prefetch(1)
    ...
    @tf.function
    def train_discriminator(real_latent, cond):
        with tf.GradientTape() as tape:
            fake_latent = generator(cond, training=True)
            d_real = discriminator([real_latent, cond], training=True)
            d_fake = discriminator([fake_latent, cond], training=True)
            gp = gradient_penalty(discriminator, real_latent, fake_latent, cond)
    ...
    @tf.function
    def train_generator(cond, real_latent):
        with tf.GradientTape() as tape:
            fake_latent = generator(cond, training=True)
            decoded = decoder(fake_latent, training=True)
            ...
            mse_latent = tf.reduce_mean(tf.square(fake_latent - real_latent))
            
            g_loss = adv_loss + LAMBDA_PHYS * physics_penalty + 0.1 * mse_latent
            
        grads = tape.gradient(g_loss, generator.trainable_variables + decoder.trainable_variables)
        g_optimizer.apply_gradients(zip(grads, generator.trainable_variables + decoder.trainable_variables))
        return g_loss, adv_loss, physics_penalty

    for epoch in range(EPOCHS_GAN):
        for step, (z_real_batch, cond_batch) in enumerate(dataset):
            for _ in range(5):  # N_CRITIC steps
                d_loss = train_discriminator(z_real_batch, cond_batch)
            g_loss, adv_loss, phys = train_generator(cond_batch, z_real_batch)

        if epoch % 50 == 0:
            print(f"Epoch {epoch}: D_loss={d_loss:.4f}, G_loss={g_loss:.4f}, Physics={phys:.4f}")

    generator.save("hybrid_generator.h5")
    discriminator.save("hybrid_discriminator.h5")
    decoder.save("decoder_finetuned_wgan.h5")
...
def evaluate_and_generate(generator, decoder, X_real, y_cond):
    ...
    norm_cond = (sampled_cond - mean) / std
    
    latent_fake = generator.predict(norm_cond, verbose=0)
    ...
    cond_tsne_norm = (cond_tsne - mean) / std
    fake_tsne = generator.predict(cond_tsne_norm, verbose=0)
...
if __name__ == "__main__":
    ...
    # 3. GAN Training with Decoder Fine-tuning
    generator, finetuned_decoder = train_gan_with_finetuning(latent_vectors, y_cond, decoder)
    
    # 4. Evaluation, Generation & Denormalization
    X_syn, y_syn = evaluate_and_generate(generator, finetuned_decoder, X_pad, y_cond)
